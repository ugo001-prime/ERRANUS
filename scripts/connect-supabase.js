...
${profileOverride}
(async()=>{localStorage.removeItem('erranusTasks');const noteText=document.querySelector('.auth-note');if(noteText)noteText.textContent='Your account uses secure Supabase sign-in, verified email and password reset. No real payments are processed yet.';const {data:{session}}=await sb.auth.getSession();sessionStorage.removeItem('erranusAccount');localStorage.removeItem('erranusSession');if(session){await useRemoteSession(session.user);if(emailConfirmationReturn)dbStatus('Email confirmed successfully. Welcome to Erranus.');}else {auth=null;$('#landing').hidden=false;$('.shell').hidden=true;render();}})();
</script>`;
