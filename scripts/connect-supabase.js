...
(async()=>{const noteText=document.querySelector('.auth-note');if(noteText)noteText.textContent='Your account uses secure Supabase sign-in, verified email and password reset. No real payments are processed yet.';const {data:{session}}=await sb.auth.getSession();sessionStorage.removeItem('erranusAccount');localStorage.removeItem('erranusSession');if(session) await useRemoteSession(session.user);else {auth=null;$('#landing').hidden=false;$('.shell').hidden=true;render();}})();
</script>`;
