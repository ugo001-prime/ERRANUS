...
(async()=>{const {data:{session}}=await sb.auth.getSession();sessionStorage.removeItem('erranusAccount');localStorage.removeItem('erranusSession');if(session) await useRemoteSession(session.user);else {auth=null;$('#landing').hidden=false;$('.shell').hidden=true;render();}})();
</script>`;
