...
function openConversation(name,taskId=current?.id){activeChatTaskId=taskId||null;
...
async function signOut(){if(taskUpdatesChannel){sb.removeChannel(taskUpdatesChannel);taskUpdatesChannel=null;}if(taskPollingTimer){clearInterval(taskPollingTimer);taskPollingTimer=null;}if(messageUpdatesChannel){sb.removeChannel(messageUpdatesChannel);messageUpdatesChannel=null;}if(messagePollingTimer){clearInterval(messagePollingTimer);messagePollingTimer=null;}if(profileUpdatesChannel){sb.removeChannel(profileUpdatesChannel);profileUpdatesChannel=null;}await sb.auth.signOut();sessionStorage.removeItem('erranusAccount');localStorage.removeItem('erranusSession');auth=null;$('#appLoader').hidden=true;$('#landing').hidden=false;$('.shell').hidden=true;$('#accountTools').innerHTML='';dbStatus('You are signed out.');}
...
async function postTask(e){e.preventDefault();if(!isClient())return dbStatus('Only Client and Dual accounts can post tasks.');const f=new FormData(e.target);const {error}=await sb.from('tasks').insert({client_id:auth.id,title:String(f.get('title')).trim(),category:String(f.get('category')).trim(),public_area:String(f.get('area')).trim(),amount:Number(f.get('amount')),duration:String(f.get('duration')).trim(),description:String(f.get('description')).trim()});if(error)return dbStatus(error.code==='23505'?'An identical active task is already posted. Open Posted tasks to manage it.':error.message);tab='posted';await loadRemoteTasks();dbStatus('Task posted for workers to find.');}
async function accept(e){e.preventDefault();if(!isWorker())return dbStatus('Only Worker and Dual accounts can accept tasks.');const taskId=current?.id,signature=String(new FormData(e.target).get('name')||'');if(!taskId)return dbStatus('Task is no longer available.');if(signature!==auth?.name)return dbStatus('Enter your full name exactly as it appears on your profile.');const {error}=await sb.rpc('accept_task',{p_task_id:taskId,p_signature:signature});if(error)return dbStatus(error.message);await loadRemoteTasks();if(tasks.some(t=>t.id===taskId))openTask(taskId);dbStatus('Agreement signed. You can now start work.');}
async function status(label){const map={'In progress':'in_progress','Completion requested':'completion_requested','Completed':'completed'},taskId=current?.id;if(!taskId)return dbStatus('Task is no longer available.');const {error}=await sb.rpc('update_task_status',{p_task_id:taskId,p_status:map[label]});if(error)return dbStatus(error.message);await loadRemoteTasks();if(tasks.some(t=>t.id===taskId))openTask(taskId);dbStatus(label==='In progress'?'Work started. Request completion when you finish.':'Task updated: '+label);}
let messageUpdatesChannel=null;
let messagePollingTimer=null;
let activeChatTaskId=null;
const closeModalForMessages=closeModal;
closeModal=function(){activeChatTaskId=null;closeModalForMessages();};
...
function setupMessageUpdates(){
  if(messageUpdatesChannel||!auth?.id)return;
  messageUpdatesChannel=sb.channel('erranus-message-updates-'+auth.id).on('postgres_changes',{event:'INSERT',schema:'public',table:'messages'},async payload=>{
    if(!tasks.some(task=>task.id===payload.new?.task_id))return;
    await refreshMessagesLive(payload.new.sender_id!==auth.id);
  }).subscribe();
  messagePollingTimer=setInterval(async()=>{
    if(auth&&!document.hidden&&(activeChatTaskId||tab==='messages'))await refreshMessagesLive(true);
  },5000);
}
...
function setupTaskUpdates(){
  ...
  taskUpdatesChannel=sb.channel('erranus-task-updates-'+auth.id).on('postgres_changes',{event:'*',schema:'public',table:'tasks'},async payload=>{
    const previousRequests=new Set(completionRequestIds);
    await loadRemoteTasks();
    const nextRequests=clientCompletionRequests();
    const newRequest=nextRequests.find(task=>!previousRequests.has(task.id));
    if(newRequest)note('Completion requested. Open Notifications to review and confirm.');
    completionRequestIds=new Set(nextRequests.map(task=>task.id));
  }).subscribe();
  taskPollingTimer=setInterval(async()=>{
    if(auth&&!document.hidden&&['dashboard','find','my','posted','completed'].includes(tab)&&!$('#modal')?.classList.contains('open')){
      await loadRemoteTasks();
    }
  },8000);
}
...
${profileOverride}
let profileUpdatesChannel=null;
function setupProfileUpdates(){
  if(profileUpdatesChannel||!auth?.id)return;
  profileUpdatesChannel=sb.channel('profile-rating-'+auth.id).on('postgres_changes',{event:'UPDATE',schema:'public',table:'profiles',filter:'id=eq.'+auth.id},payload=>{
    const rating=Number(payload.new?.rating||0),completed=Number(payload.new?.completed_task_count||0),accounts=getAccounts(),account=accounts[auth.email]||{};
    accounts[auth.email]={...account,rating,completed}; localStorage.erranusAccounts=JSON.stringify(accounts);
    if(tab==='profile')render();
    note('Your worker rating has been updated.');
  }).subscribe();
}
async function review(e){
  e.preventDefault();
  const taskId=current?.id,rating=Number(new FormData(e.target).get('rating')),comment=String(new FormData(e.target).get('comment')||'').trim();
  if(!taskId||!isClient())return dbStatus('Only the client who posted this completed task can leave a worker rating.');
  const {error}=await sb.rpc('submit_worker_review',{p_task_id:taskId,p_rating:rating,p_comment:comment});
  if(error)return dbStatus(error.code==='23505'?'A worker rating has already been submitted for this task.':error.message);
  closeModal(); await loadRemoteTasks(); dbStatus('Worker rating saved and updated immediately.');
