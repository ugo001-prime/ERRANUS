...
async function accept(e){e.preventDefault();if(!isWorker())return dbStatus('Only Worker and Dual accounts can accept tasks.');const taskId=current?.id;if(!taskId)return dbStatus('Task is no longer available.');const {error}=await sb.rpc('accept_task',{p_task_id:taskId,p_signature:String(new FormData(e.target).get('name')).trim()});if(error)return dbStatus(error.message);await loadRemoteTasks();if(tasks.some(t=>t.id===taskId))openTask(taskId);dbStatus('Agreement signed. You can now start work.');}
