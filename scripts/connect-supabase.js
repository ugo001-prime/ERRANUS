...
function messagesInbox(){
  ...
  const cards=conversations.map(c=>'<button class="card click" type="button" onclick="openConversation(null,&quot;'+c.id+'&quot;)"><div class="row"><span class="account-avatar">'+safe(initials(c.person.name))+'</span><span><b>'+safe(c.person.name)+'</b><small class="small">@'+safe(c.person.username)+' · '+safe(taskConversationName(c.task))+'</small></span></div><p class="small">'+safe(c.last?.body||'')+'</p><span class="small">'+c.count+' message'+(c.count===1?'':'s')+' · Open chat →</span></button>').join('');
  return '<div class="ey">MESSAGES</div><h1 class="title">Your chats</h1><p class="intro">Select a person to open your recorded task conversation.</p>'+(cards?'<div class="grid">'+cards+'</div>':'<div class="card">No chats yet. Open an assigned task and select Message to start a conversation.</div>')+(available.length?'<div class="banner"><b>Task messaging</b><p class="small">Chats are visible only to the client and assigned worker.</p></div>':'');
}
function openConversation(name,taskId=current?.id){
  ...
  const body=thread.map(m=>'<p class="small"><b>'+ (m.sender_id===auth?.id?'You':safe(person.name)) +':</b> '+safe(m.body)+'</p>').join('')||'<p class="small">No messages yet. Start the conversation.</p>';
  $('#dialog').innerHTML='<button class="secondary close" onclick="closeModal()">Close</button><div class="ey">TASK CHAT</div><h1 class="title">'+safe(person.name)+'</h1><p class="small">@'+safe(person.username)+' · '+safe(taskConversationName(task))+'</p><div class="card">'+body+'</div><form class="form" style="margin-top:14px" onsubmit="sendMessage(event,&quot;'+taskId+'&quot;)"><label class="field">Message<textarea name="message" required maxlength="2000" placeholder="Write a message about this task..."></textarea></label><button>Send message</button></form>';
...
${profileOverride}
(async()=>{localStorage.removeItem('erranusTasks');const noteText=document.querySelector('.auth-note');if(noteText)noteText.textContent='Your account uses secure Supabase sign-in, verified email and password reset. No real payments are processed yet.';const {data:{session}}=await sb.auth.getSession();sessionStorage.removeItem('erranusAccount');localStorage.removeItem('erranusSession');if(session){if(passwordRecoveryReturn){auth=null;$('#accountTools').innerHTML='';$('#landing').hidden=false;$('.shell').hidden=true;showRecoveryPassword();}else {await useRemoteSession(session.user);if(emailConfirmationReturn)dbStatus('Email confirmed successfully. Welcome to Erranus.');}}else {auth=null;$('#landing').hidden=false;$('.shell').hidden=true;render();}})();
</script>`;
